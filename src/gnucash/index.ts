...
export async function parseGnucash(source: GnucashSource): Promise<Gnucash> {
  ...
  return {
    books: toIdMap(data["gnc-v2"]["gnc:book"].map(parseBook))
  };
}
...
function parseBook(data: any): Book {
  return {
    id: data["book:id"][0]._,
    commodities: toIdMap(data["gnc:commodity"].map(parseCommodity)),
    accounts: toIdMap(data["gnc:account"].map(parseAccount)),
    transactions: toIdMap(data["gnc:transaction"].map(parseTransaction))
  };
}
...
function toIdMap<E extends Identifiable>(entities: E[]): IdMap<E> {
  const map: IdMap<E> = {};
